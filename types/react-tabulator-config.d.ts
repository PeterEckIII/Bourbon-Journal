// export type definitions for tabulator-tables 5.0
// Project: http://info
...
export interface OptionsCells extends CellCallbacks {
  ...
  validationFailed?:
    | ((
        cell: CellComponent,
        value: any,
        validators: Validator[] | StandardValidatorType[]
      ) => void)
    | undefined;
}
...
export interface OptionsClipboard {
  ...
  /**You can alter the finished output to the clipboard using the clipboardCopyFormatter callback. The callback function receives two arguments, the first is a string representing the export type of content to be formatted (either "plain" or "html" depending on the export type of data entering the clipboard). The second argument is the string that is about to be insered into the clipboard. The function and should return a string that will be inserted into the clipboard */
  clipboardCopyFormatter?:
    | "table"
    | ((type: "plain" | "html", output: string) => string)
    | undefined;
...
export interface OptionsPersistentConfiguration {
  ...
  /**The persistenceWriterFunc function will receive three arguments, the persistance id of the table, the export type of data to be written and an object or array representing the data */
  persistenceWriterFunc?:
    | ((id: string, type: keyof PersistenceOptions, data: any) => any)
    | undefined;
  /**The persistenceReaderFunc function will receive two arguments, the persistance id of the table, and the export type of data to be written. This function must synchronously return the data in the format in which it was passed to the persistenceWriterFunc function. It should return a value of false if no data was present */
  persistenceReaderFunc?:
    | ((id: string, type: keyof PersistenceOptions) => any)
    | undefined;
}
...
export interface OptionsRowGrouping {
  ...
  groupVisibilityChanged?:
    | ((group: GroupComponent, visible: boolean) => void)
    | undefined;
...
export interface OptionsFiltering {
  ...
  dataFiltered?:
    | ((filters: Filter[], rows: RowComponent[]) => void)
    | undefined;
...
export interface OptionsData {
  ...
  /** The HTTP request export type for Ajax requests or config object for the request     */
  ajaxConfig?: HttpMethod | AjaxConfig | undefined;

  /** When using a request method other than "GET" Tabulator will send any parameters with a content export type of form data. You can change the content export type with the ajaxContentexport type option. This will ensure parameters are sent in the format you expect, with the correct headers. * * The ajaxContentexport type option can take one of two values:
          "form" - send parameters as form data (default option)
          "json" - send parameters as JSON encoded string
          If you want to use a custom content export type then you can pass a content export type formatter object into the ajaxContentexport type option. this object must have two properties, the headers property should contain all headers that should be sent with the request and the body property should contain a function that returns the body content of the request
      */
  ...
  ajaxURLGenerator?:
    | ((url: string, config: any, params: any) => string)
    | undefined;

  /** callback function to replace inbuilt ajax request functionality     */
  ajaxRequestFunc?:
    | ((url: string, config: any, params: any) => Promise<any>)
    | undefined;
  ...
  ajaxError?:
    | ((xhr: any, textStatus: any, errorThrown: any) => void)
    | undefined;
...
export interface OptionsRows {
  ...
  movableRowsElementDrop?:
    | ((e: MouseEvent, element: HTMLElement, row: RowComponent) => any)
    | undefined;
  ...
  dataTreeRowExpanded?:
    | ((row: RowComponent, level: number) => void)
    | undefined;
  ...
  movableRowsSent?:
    | ((fromRow: RowComponent, toRow: RowComponent, toTable: Tabulator) => void)
    | undefined;
  ...
  movableRowsReceivingStart?:
    | ((fromRow: RowComponent, toTable: Tabulator) => void)
    | undefined;
  ...
  rowSelectionChanged?:
    | ((data: any[], rows: RowComponent[]) => void)
    | undefined;
...
export interface OptionsColumns {
  ...
  columnVisibilityChanged?:
    | ((column: ColumnComponent, visible: boolean) => void)
    | undefined;
...
export type RowContextMenuSignature =
  ...
  | ((
      ...
      e: MouseEvent
    ) => MenuObject<RowComponent> | false | any[]);
...
export type GroupContextMenuSignature =
  ...
  | ((
      ...
      e: MouseEvent
    ) => MenuObject<GroupComponent> | false | any[]);

export interface MenuObject<
  T extends RowComponent | CellComponent | ColumnComponent | GroupComponent
> {
...
export interface OptionsHTML {
  ...
  printFormatter?:
    | ((tableHolderElement: any, tableElement: any) => any)
    | undefined;
...
export type StandardStringParam =
  | string
  | HTMLElement
  | (() => string | HTMLElement);

export interface AddditionalExportOptions {
...
export type HistoryAction = "cellEdit" | "rowAdd" | "rowDelete" | "rowMoved";
...
export interface ColumnDefinition extends ColumnLayout, CellCallbacks {
  ...
  /**  By default Tabulator will try and match the comparison export type to the export type of element used for the header filter.
  Standard input elements will use the "like" filter, this allows for the matches to be displayed as the user types.
  For all other element types (select boxes, check boxes, input elements of export type number) an "=" filter export type is used.
  If you want to specify the export type of filter used you can pass it to the headerFilterFunc option in the column definition object. This will take any of the standard filters outlined above or a custom function*/
  headerFilterFunc?:
    | FilterType
    | ((
        headerValue: any,
...
export type GlobalTooltipOption = boolean | ((cell: CellComponent) => string);
...
export type CustomMutatorParams =
  | {}
  | ((
      value: any,
      data: any,
...
export type ScrollToRowPostition = "top" | "center" | "bottom" | "nearest";
...
export interface SharedEditorParams {
  ...
  /**Built in editors based on input elements such as the input, number, textarea and autocomplete editors have the ability to mask the users input to restrict it to match a given pattern.
      ...
      Each character in the string passed to the mask option defines what export type of character can be entered in that position in the editor.
  ...
  /** you are using fixed characters in your mask (any character other that A, 9 or *), then you can get the mask to automatically fill in these characters for you as you export type by setting the maskAutoFill option in the editorParams to true */
...
export interface InputParams extends SharedEditorParams {
  /**Changes input export type to 'search' and shows an 'X' clear button to clear the cell value easily */
...
export interface AutoCompleteParams
  ...
  listItemFormatter?: ((value: string, text: string) => string) | undefined;
  searchFunc?:
    | ((term: string, values: string[]) => string[] | Promise<string[]>)
    | undefined;
  allowEmpty?: boolean | undefined;
...
export type StandardValidatorType =
  | "required"
  | "unique"
  | "integer"
  | "float"
...
export type ColumnSorterParamLookupFunction = (
  column: ColumnComponent,
  dir: SortDirection
) => {};
...
// prototype.(?!registerModule|helpers|_)\w+
declare class Tabulator {
  ...
  /** You have a choice of four file types to choose from:
    ...
    To trigger a download, call the download function, passing the file export type (from the above list) as the first argument, and an optional second argument of the file name for the download (if this is left out it will be "ext"). The optional third argument is an object containing any setup options for the formatter, such as the delimiter choice for CSV's).
    ...
    If you want to create a custom file export type from the table data then you can pass a function to the export type argument, instead of a string value. At the end of this function you must call the setFileContents function, passing the formatted data and the mime type.
    */
  download: (
    downloadType:
      | DownloadType
  ...
  You can allow any other export type of file into the file picker by passing the extension or mime export type into the first argument of the setDataFromLocalFile function as a comma separated list. This argument will accept any of the values valid for the accept field of an input element */
...
*/
  ...
  */
  getRowPosition: (row: RowLookup, activeOnly?: boolean) => number;
 ...
 */
  ...
  moveColumn: (
    fromColumn: ColumnLookup,
    toColumn: ColumnLookup,
    after: boolean
  ) => void;
...
*/
  scrollToColumn: (
    column: ColumnLookup,
    position?: ScrollToColumnPosition,
    ifVisible?: boolean
  ) => Promise<void>;
  ...
  clearSort: () => void;
  /** To set a filter you need to call the setFilter method, passing the field you wish to filter, the comparison export type and the value to filter for.
 ...
 */
  ...
  on: <K extends keyof EventCallBackMethods>(
    event: K,
    callback?: EventCallBackMethods[K]
  ) => void;
  off: <K extends keyof EventCallBackMethods>(
    event: K,
    callback?: EventCallBackMethods[K]
  ) => void;
}
